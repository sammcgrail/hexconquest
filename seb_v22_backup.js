function decideActions(state) {
  ...
  for (var i = 0; i < state.hexes.length; i++) {
    ...
    if (h.hasKing && h.owner !== null && h.owner !== me && state.players[h.owner].alive)
      enemyKings.push({idx: h.owner, q: h.q, r: h.r, units: h.units, terr: state.players[h.owner].territory, totalUnits: state.players[h.owner].totalUnits});
  }
  ...
  // Build threat map — total enemy units adjacent to each of our hexes
  var threatMap = {};
  var kingThreat = 0;
  for (var i = 0; i < state.hexes.length; i++) {
    var h = state.hexes[i];
    ...
    var threat = 0;
    for (var d = 0; d < dirs.length; d++) {
      var nh = get(h.q+dirs[d][0], h.r+dirs[d][1]);
      if (nh && nh.owner !== null && nh.owner !== me && nh.units > 0) threat += nh.units;
    }
    if (threat > 0) threatMap[key(h.q, h.r)] = threat;
    if (h.hasKing) kingThreat = threat;
  }

  // King guard ring — hexes adjacent to king that we own
  var guardRingKeys = {};
  var guardRingUnits = 0;
  guardRingKeys[key(KQ, KR)] = true;
  for (var d = 0; d < dirs.length; d++) {
    var nh = get(KQ+dirs[d][0], KR+dirs[d][1]);
    if (nh && nh.owner === me) {
      guardRingKeys[key(nh.q, nh.r)] = true;
      guardRingUnits += nh.units;
    }
  }
  ...
  for (var mi = 0; mi < state.legalMoves.length; mi++) {
    ...
    // KING: only flee when directly threatened
    if (isKing) {
      if (kingThreat > 0) {
    ...
    // Don't strip guard ring when threatened
    var inGuardRing = guardRingKeys[key(m.from.q, m.from.r)];
    if (inGuardRing && kingThreat > 0 && from.units <= 3) continue;
    ...
    for (var ai = 0; ai < m.adjacent.length; ai++) {
      ...
      if (t.owner === null) {
        // EXPAND with clustering (BMO-style)
        sc = earlyGame ? 22 : 14;
        if (t.type === 'resource') sc += 35;
        sc += fn * 6;
        if (en > 0) sc += fn * 4;
        ...
        if (t.hasKing && movable > needed) {
          ...
          sc = 18;
          if (t.type === 'resource') sc += 15;
          if (weakest && t.owner === weakest.idx) sc += 12;
          sc += fn * 3;
          // Don't attack early unless very easy
          if (earlyGame && t.units > 1) sc = -10;
        } else {
          sc = -50;
        }

      } else {
        // OWN HEX — reinforce frontlines
        if (en > 0) sc = 5;
        else {
      ...
      // KING HUNTING — approach enemy kings mid/late game
      if ((midGame || lateGame) && enemyKings.length > 0) {
        for (var ki = 0; ki < enemyKings.length; ki++) {
          ...
          // Flank bonus — approach from edges when close
  ...
  while (gold >= 10) {
    ...
    for (var mi = 0; mi < state.legalMoves.length; mi++) {
      ...
      for (var ai = 0; ai < m.adjacent.length; ai++) {
        ...
        if (adj && adj.hasKing && adj.owner !== me) ss += 80;
      }
      var kd2 = dist(m.from.q, m.from.r, KQ, KR);
      if (kingThreat > 0 && kd2 <= 2) ss += 40;
      if (kd2 <= 2) ss += 6;
      if (lateGame && weakest) {
        var ekd = dist(m.from.q, m.from.r, weakest.q, weakest.r);
